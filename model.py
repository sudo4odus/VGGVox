...
def test():
	...
	functor = [K.function([inp, K.learning_phase()], [out]) for out in outputs]     # evaluation functions

	# Testing
	#print('####################### {}'.format(inp.shape))
	test = np.random.random(inp.shape[1])[np.newaxis,:]
	layer_outs = [func([test]) for func in functor]
	#layer_outs = functor([test, 1.])

	for i in range(11):
		print("Shape of layer {} output:{}".format(i, layer_outs[i].shape))
